...
import { AppUser } from "../types/auth"; // this import is healthy to prevent future risky even typescript infer AppUser from global express

// Removed AuthRequest and CustomerAuthRequest in favor of global Express.Request with AppUser
  ...
  return async (
    ...
    try {
      ...
      if (options.type === "employee") {
        ...
        req.user = { ...employee, kind: "employee" };
      } else if (options.type === "customer") {
        const customerResult = await customerService.getCustomerById(
          decoded.userId,
        );
        ...
        req.user = { ...customer, kind: "customer" };
      }
...
export const authorize = (allowedRoles: string[]) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (user.kind !== "employee" || !allowedRoles.includes(user.role)) {
      res.status(403).json({
        success: false,
        message: "Insufficient permissions",
        required: allowedRoles,
        current: user.kind === "employee" ? user.role : "CUSTOMER",
      });
      return;
    }
